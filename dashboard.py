...
MATERIAS_PRIMAS = {
    ...
    "RB=F": "Gasolina", "HO=F": "Aceite calefacción", "ZR=F": "Arroz",
    "ZO=F": "Avena", "ZL=F": "Aceite de soja", "ZM=F": "Harina de soja"
}
...
def get_precio_finnhub(simbolo):
    try:
        ...
        d = r.json()
        return {"precio": d.get("c", 0), "cambio_pct": d.get("dp", 0)}
    except:
        return None
...
def get_precio_binance(simbolo):
    try:
        ...
        d = r.json()
        return {"precio": float(d.get("lastPrice", 0)), "cambio_pct": float(d.get("priceChangePercent", 0))}
    except:
        return None
...
def render_tarjeta(simbolo, nombre, precio_data):
    ...
    <div class="metric-card" style="margin-bottom:8px;">
        <div style="display:flex; justify-content:space-between; margin-bottom:6px;">
            ...
            <div style="color:{color}; font-size:11px;">{signo}{cambio:.2f}%</div>
        </div>
        <div style="color:#8B8BAA; font-size:11px; margin-bottom:6px;">{nombre}</div>
        <div style="color:#F0EEFF; font-size:16px; font-weight:600;">{precio_str}</div>
    </div>
    """, unsafe_allow_html=True)
    return True
...
with st.sidebar:
    st.markdown("""
    <div style="padding:16px 0 20px; display:flex; align-items:center; gap:10px;">
    ...
    st.markdown("""
    <div style="background:#12122A;border:1px solid #2D2D4A;border-radius:8px;padding:8px 12px;text-align:center;">
...
if pagina == "Inicio":
    ...
    with c1:
        st.markdown("""<div class="metric-card" style="text-align:center;padding:24px 16px;">
    ...
    with col_n:
        st.markdown("""<div class="metric-card" style="text-align:center;padding:20px 16px;">
    ...
    if tab == "Acciones":
        ...
        for i, (simbolo, nombre) in enumerate(ACCIONES.items()):
            with cols[i % 4]:
                ...
                if render_tarjeta(simbolo, nombre, data):
                    if st.button("Analizar", key=f"act_{simbolo}"):
                        st.session_state.pagina = "Análisis"
        ...
        for i, (simbolo, nombre) in enumerate(DIVISAS.items()):
            with cols[i % 4]:
                ...
                if render_tarjeta(simbolo, nombre, data):
                    if st.button("Analizar", key=f"div_{simbolo}"):
                        st.session_state.pagina = "Análisis"
                        st.session_state.analizar_simbolo = simbolo
                        st.session_state.analizar_nombre = nombre
                        st.rerun()
        ...
        for i, (simbolo, nombre) in enumerate(MATERIAS_PRIMAS.items()):
            with cols[i % 4]:
                data = get_precio_yahoo(simbolo)
                if render_tarjeta(simbolo, nombre, data):
                    if st.button("Analizar", key=f"mat_{simbolo}"):
                        st.session_state.pagina = "Análisis"
                        st.session_state.analizar_simbolo = simbolo
                        st.session_state.analizar_nombre = nombre
                        st.rerun()
        ...
        for i, (simbolo, nombre) in enumerate(CRIPTOS_BINANCE.items()):
            with cols[i % 4]:
                ...
                if render_tarjeta(simbolo, nombre, data):
                    if st.button("Analizar", key=f"cri_{simbolo}"):
                        st.session_state.pagina = "Análisis"
                        st.session_state.analizar_simbolo = simbolo.replace("USDT", "-USD")
                        st.session_state.analizar_nombre = nombre
                        st.rerun()
...
# ============================================================
    ...
    if not simbolo_default:
        st.markdown("""
        <div style="text-align:center;padding:48px 20px;background:#1A1A2E;border-radius:12px;border:1px solid #2D2D4A;margin-bottom:24px;">
            <div style="font-size:40px;margin-bottom:16px;">🔍</div>
            <div style="color:#F0EEFF;font-size:18px;font-weight:600;margin-bottom:8px;">Analiza cualquier activo</div>
            <div style="color:#8B8BAA;font-size:13px;line-height:1.7;max-width:420px;margin:0 auto;">
    ...
    with col1:
        simbolo = st.text_input("Símbolo del activo", value=simbolo_default, placeholder="AAPL, TSLA, EURUSD=X, GC=F, BTC-USD...")
    with col2:
        nombre_empresa = st.text_input("Nombre del activo", value=nombre_default, placeholder="Apple, Tesla, Euro/Dólar...")
    ...
    if analizar and simbolo:
        st.session_state.pop("analizar_simbolo", None)
        st.session_state.pop("analizar_nombre", None)
        ...
        with st.spinner(f"Analizando {simbolo}..."):
            ...
            try:
                resultado = newsapi.get_everything(
                    ...
                    language="en", sort_by="publishedAt", from_param=fecha_desde, page_size=10,
                    sources="bbc-news,reuters,cnn,cnbc,the-guardian-uk,associated-press,the-wall-street-journal,bloomberg,marketwatch"
                )
        ...
        with st.spinner("Generando análisis con IA..."):
            ...
            prompt = f"""Eres un analista financiero experto. Analiza {nombre_empresa} ({simbolo}) y genera exactamente un párrafo de máximo 4 oraciones en español. Sin markdown, sin asteriscos, sin símbolos especiales. Solo texto plano profesional.

Datos: Precio ${precio:.2f}, RSI {rsi:.1f}, Tendencia {'alcista' if ma20 > ma50 else 'bajista'}, MACD {'positivo' if macd > macd_sig else 'negativo'}, Bollinger {'bajo banda inferior' if precio < bb_inf else 'sobre banda superior' if precio > bb_sup else 'dentro de bandas'}, Sentimiento {'positivo' if sentimiento > 0.1 else 'negativo' if sentimiento < -0.1 else 'neutral'}, Recomendación: {decision} con {confianza}% confianza."""
        ...
        fig.add_trace(go.Candlestick(
            x=datos.index, open=datos['Open'].squeeze(),
            high=datos['High'].squeeze(), low=datos['Low'].squeeze(),
        ...
        with col_rsi:
            ...
            fig_rsi = go.Figure()
            fig_rsi.add_trace(go.Scatter(x=datos.index, y=datos['RSI'], name="RSI",
            ...
            fig_rsi.update_layout(height=280, template="plotly_dark", paper_bgcolor="#1A1A2E",
                                   plot_bgcolor="#1A1A2E", yaxis=dict(range=[0, 100]),
                                   margin=dict(l=10, r=10, t=20, b=10))
        ...
        with col_macd:
            ...
            fig_macd.add_trace(go.Bar(x=datos.index, y=datos['MACD_hist'], name="Histograma",
                                       marker_color=['#4ADE80' if val > 0 else '#F87171' for val in datos['MACD_hist']]))
            fig_macd.update_layout(height=280, template="plotly_dark", paper_bgcolor="#1A1A2E",
        ...
        fig_bb.add_trace(go.Scatter(x=datos.index, y=datos['BB_inferior'], name="Banda inferior",
        ...
        fig_bb.update_layout(height=340, template="plotly_dark", paper_bgcolor="#1A1A2E",
                              plot_bgcolor="#1A1A2E", margin=dict(l=10, r=10, t=20, b=10))
                    ...
                    with ci:
                        if imagen_url and imagen_url.startswith('http'):
                            try:
                                ...
                                st.markdown("""<div style="width:120px;height:80px;background:#2D2D4A;border-radius:8px;display:flex;align-items:center;justify-content:center;color:#8B8BAA;font-size:20px;">📰</div>""", unsafe_allow_html=True)
                        else:
                            st.markdown("""<div style="width:120px;height:80px;background:#2D2D4A;border-radius:8px;display:flex;align-items:center;justify-content:center;color:#8B8BAA;font-size:20px;">📰</div>""", unsafe_allow_html=True)
                    with ct:
...
# ============================================================
    ...
    FUENTES = "bbc-news,reuters,cnn,cnbc,the-guardian-uk,associated-press,the-wall-street-journal,marketwatch,bloomberg"

    with st.spinner("Cargando noticias..."):
        fecha_desde = (datetime.now() - timedelta(days=3)).strftime('%Y-%m-%d')
        try:
            resultado = newsapi.get_everything(
                q=queries[categoria], language="en", sort_by="publishedAt",
                from_param=fecha_desde, page_size=20, sources=FUENTES
            )
            noticias_globales = resultado.get('articles', [])
        except:
            resultado = newsapi.get_everything(
                q=queries[categoria], language="en", sort_by="publishedAt",
                from_param=fecha_desde, page_size=20
            )
            noticias_globales = resultado.get('articles', [])
...
# ============================================================
    ...
    if tipo == "Divisas":
        ...
        if st.button("Convertir", type="primary"):
            with st.spinner("Obteniendo tasa..."):
                if moneda_de == moneda_a:
                    tasa = 1.0
                elif moneda_de == "USD":
                    tasa = get_tasa_conversion("USD", moneda_a)
                elif moneda_a == "USD":
                    t = get_tasa_conversion("USD", moneda_de)
                    tasa = 1 / t if t else None
                else:
                    t1 = get_tasa_conversion("USD", moneda_de)
                    t2 = get_tasa_conversion("USD", moneda_a)
                    tasa = (t2 / t1) if t1 and t2 else None
            ...
            if tasa:
                ...
                <div class="rec-card" style="text-align:center;padding:28px;">
                    ...
                    <div style="color:#F0EEFF;font-size:32px;font-weight:600;margin-bottom:8px;">
                        {monto:,.2f} {moneda_de} = <span style="color:#A78BFA;">{res:,.4f} {moneda_a}</span>
                    </div>
                ...
                st.error("No se pudo obtener la tasa. Intenta de nuevo.")
        ...
        with col2:
            st.markdown("<br><br><div style='text-align:center;color:#A78BFA;font-size:24px;'>→</div>", unsafe_allow_html=True)
        with col3:
            st.markdown("<br><br><div style='color:#F0EEFF;font-size:14px;padding-top:8px;'>USD (Dólares)</div>", unsafe_allow_html=True)

        if st.button("Convertir", type="primary"):
            with st.spinner("Obteniendo precio..."):
                data = get_precio_binance(f"{cripto}USDT")
            if data:
                res = monto * data['precio']
                st.markdown(f"""
                <div class="rec-card" style="text-align:center;padding:28px;">
                    <div style="color:#8B8BAA;font-size:13px;margin-bottom:8px;">Resultado</div>
                    <div style="color:#F0EEFF;font-size:32px;font-weight:600;margin-bottom:8px;">
                        {monto:,.6f} {cripto} = <span style="color:#A78BFA;">${res:,.2f} USD</span>
                    </div>
                    <div style="color:#8B8BAA;font-size:13px;">1 {cripto} = ${data['precio']:,.2f} USD</div>
        ...
        with col1:
            ...
            monto = st.number_input("Cantidad", min_value=0.0, value=1.0, step=0.01)
        with col2:
            st.markdown("<br><br><div style='text-align:center;color:#A78BFA;font-size:24px;'>→</div>", unsafe_allow_html=True)
        with col3:
            st.markdown("<br><br><div style='color:#F0EEFF;font-size:14px;padding-top:8px;'>USD (Dólares)</div>", unsafe_allow_html=True)
        ...
        if st.button("Convertir", type="primary"):
            ...
            if data:
                ...
                <div class="rec-card" style="text-align:center;padding:28px;">
                    <div style="color:#8B8BAA;font-size:13px;margin-bottom:8px;">Resultado</div>
                    <div style="color:#F0EEFF;font-size:32px;font-weight:600;margin-bottom:8px;">
                        {monto:,.2f} unidades de {materia} = <span style="color:#A78BFA;">${res:,.2f} USD</span>
                    </div>
                    <div style="color:#8B8BAA;font-size:13px;">1 unidad = ${data['precio']:,.2f} USD</div>
...
# ============================================================
    ...
    with c1:
        st.markdown("""<div class="metric-card">
            ...
            <div style="color:#8B8BAA;font-size:12px;line-height:1.9;">Finnhub API — acciones<br>Yahoo Finance — divisas y materias primas<br>Binance API — criptomonedas<br>NewsAPI — noticias globales</div>
        </div>""", unsafe_allow_html=True)
    with c2:
        st.markdown("""<div class="metric-card">
            <div style="color:#A78BFA;font-size:13px;font-weight:600;margin-bottom:10px;">Tecnología</div>
            <div style="color:#8B8BAA;font-size:12px;line-height:1.9;">Python 3.14<br>Streamlit<br>Claude AI (Anthropic)<br>Plotly · Pandas</div>
        </div>""", unsafe_allow_html=True)
    with c3:
        st.markdown("""<div class="metric-card">
            <div style="color:#A78BFA;font-size:13px;font-weight:600;margin-bottom:10px;">Fuentes de noticias</div>
            <div style="color:#8B8BAA;font-size:12px;line-height:1.9;">BBC News · Reuters · CNN<br>CNBC · The Guardian<br>Associated Press<br>Wall Street Journal · Bloomberg</div>
        </div>""", unsafe_allow_html=True)
    ...
    <div style="background:#1A1A0D;border:1px solid #3D3A00;border-radius:10px;padding:20px 24px;">
        ...
        <div style="color:#D4C570;font-size:12px;line-height:1.9;">
            ...
            y la regulación de la Superintendencia Financiera de Colombia (SFC) — la prestación 
            de asesoramiento financiero profesional requiere autorización y registro ante las 
            entidades competentes.<br><br>
